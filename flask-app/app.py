#pip install tensorflow"C:\Users\My PC\Downloads\fyp-wei-ting\fyp-wei-ting\Updated for HTML"
#pip install flask
#
import sys
sys.path
...
import requests, json, random, ast, re
from types import SimpleNamespace

# Create application, note that template folder must contain all the html/php files that will be used
app = Flask(__name__, template_folder='C:/Users/My PC/trafficprediction/templates')
...
model = load_model('C:/Users/My PC/trafficprediction/trafficANN.h5')

# Load the dataset
df = pd.read_csv('C:/Users/My PC/trafficprediction/combined_data.csv')
...
def timePredict(road_type, prediction, prediction2, distance, duration, routes):
    ...
    for i in range(len(prediction)):
        ...
        for j in range(len(prediction2)):
            ...
            est_time_added = est_time_added + abs(max_time - min_time)
...
def dashboard():
    ...
    return render_template('report.html', liTraffic = liTraffic, liRoutes = liRoutes)
...
def alarm():
    ...
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    sql = "INSERT INTO schedule_emails (recipient, subject, message, schedule_time, schedule_date) VALUES (%s, %s, %s, %s, %s)"
    val = (recipient, subject, message, schedule_time, schedule_date)
...
def predict():
    ...
    if request.method == "POST":
        ...
        if request.form["submit"] == "Predict":
            ...
            # Render the result HTML template with the predicted traffic volume
            if prediction2 != "":
                return render_template('mainPython.html', result = prediction, road = prediction2, time = timePred)
            else:
                return render_template('mainPython.html', result = prediction)
